...
function sigmaLimits( sigmacanvas ) {    
    ...
    $( sigmacanvas ).width($("#sigma-example").parent().parent().parent().width());
    $( sigmacanvas ).height($("#sigma-example").parent().parent().parent().height()-4);
...
function The_Main( mainfile , source) { 
    ...
    if(RES["OK"]) {
        ...
        print( sigmaLimits('#sigma-example') );
        $( window ).resize(function() { print(sigmaLimits('#sigma-example')) });
        // [ / Initiating Sigma-Canvas ]
